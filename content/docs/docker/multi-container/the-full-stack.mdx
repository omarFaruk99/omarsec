...
Compose does not require your services to live in one folder. A common layout keeps each service in its own subfolder, each with its own Dockerfile, and the `compose.yaml` at the root tying them together:
