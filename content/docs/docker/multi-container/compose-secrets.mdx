...
A **secret** in Compose is a value stored in its own file, mounted into a container as a file — not as a plain environment variable — and kept out of the parts of Docker that casually display environment variables.
...
Notice `POSTGRES_PASSWORD_FILE`, not `POSTGRES_PASSWORD`. The official Postgres image specifically supports a `_FILE`-suffixed version of its environment variables — instead of reading the password directly, it reads the **path** to a file, then reads the password from inside that file at startup.

Compose mounts your `secrets/db_password.txt` file at `/run/secrets/db_password` inside the container automatically — that path comes from the `secrets:` section at the bottom of the file, not from anything you configure manually.
...
Compose secrets are a real improvement over plain environment variables, but they are still local files on disk — this is not the same as a managed secrets service like a cloud provider's secrets manager. For a small team or a personal project, this is a solid, practical step up. Production-grade secrets management is covered in the Production & Deploy section.
</Callout>
