...
<Callout type="info">
  **`chmod 600` কী করে?** এটি file-এর permission এমনভাবে সেট করে যে শুধু file-এর owner (আপনার `aladin` user) পড়তে এবং লিখতে পারবে। অন্য কোনো user এই file দেখতে পারবে না।
...
`.env` ফাইলটি সঠিকভাবে তৈরি হয়েছে কিনা দেখুন:

```bash filename="SERVER — Terminal"
cat .env
```

আপনার লেখা variable-গুলো দেখতে পাবেন।

<Callout type="info">
  **Next.js কখন `.env` পড়ে?** Next.js নিজে থেকেই project root-এর `.env` ফাইল পড়ে — কোনো extra package (যেমন `dotenv`) install করতে হয় না। এটি `next build` এবং `next start` — দুই সময়েই কাজ করে।

  তাই এখানে আলাদা করে test করার দরকার নেই। পরের page-এ PM2 দিয়ে app চালু করার পর `pm2 logs myapp` দেখলেই বুঝবেন variable ঠিকমতো load হয়েছে কিনা।
</Callout>

<Callout type="warning">
  **`NODE_ENV` কখনো `.env`-এ লিখবেন না।** Next.js এটি নিজে সেট করে — `npm run dev` দিলে `development`, `npm run build` ও `npm run start` দিলে `production`। `.env`-এ লিখলে Next.js সেটি উপেক্ষা করে এবং একটি warning দেখায়।
</Callout>
