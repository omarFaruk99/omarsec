...
<Callout type="default">
  AWS-এ আপনি `ubuntu` user হিসেবে আছেন যার `sudo` access আছে — সরাসরি root নন। এটিই আপনার কাজের user, আলাদা করে নতুন user বানানোর দরকার নেই। পরের page-এ এই user-এ SSH key বসিয়ে server secure করব।
</Callout>
...
**পরবর্তী →** [Secure the Server](/docs/server-deployment/foundation/secure-server)

<span style={{ display: 'none' }}>
  contabo server setup bangla, aws ec2 free tier bangla, vps vs cloud bangla, ubuntu server create, server deploy bangla, vps provider comparison bangla, aws ec2 ubuntu login, server user setup bangla
</span>
