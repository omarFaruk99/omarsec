...
PM2 দিয়ে app চালু করার আগে অবশ্যই **build** করতে হবে।

<Callout type="warning">
  `npm run start` কোনো code compile করে না — এটি শুধু `.next` folder-এ থাকা তৈরি করা production build চালায়। Build না করে `pm2 start` দিলে app চালু হবে না এবং PM2-তে `errored` status দেখাবে।

  Error টি হবে: `Could not find a production build in the '.next' directory`
</Callout>
...
`npm install` আপনার project-এর সব dependency নামিয়ে আনে, আর `npm run build` source code-কে optimized production bundle-এ রূপান্তর করে `.next` folder-এ রাখে।

### Start the App

এবার PM2 দিয়ে app চালু করুন:
...
`status` column-এ `online` দেখলে app সফলভাবে চলছে। `errored` দেখলে বা `restart` count বাড়তে থাকলে `pm2 logs myapp` দিয়ে কারণ দেখুন।
...
sudo env PATH=$PATH:/home/aladin/.nvm/versions/node/v20.x.x/bin pm2 startup systemd -u aladin --hp /home/aladin
```
...
<Callout type="warning">
  উপরের output-এ লক্ষ্য করুন path-এ Node-এর **version number** লেখা আছে (`v20.x.x`)। ভবিষ্যতে `nvm install --lts` দিয়ে Node upgrade করলে এই path বদলে যাবে এবং পুরনো path আর থাকবে না।

  ফলে server reboot হলে PM2 আপনার app চালু করতে পারবে না — কোনো error ছাড়াই site down হয়ে যাবে। **Node upgrade করার পর সবসময়** `pm2 unstartup` দিয়ে পুরনো setting মুছে আবার `pm2 startup` চালাবেন।
</Callout>
