...
ফলাফল হাতেনাতে পাওয়া যায় — সাধারণ Next.js image প্রায় ১ GB, standalone দিয়ে সেটি ১৫০ MB-র কাছাকাছি নেমে আসে।

<Callout type="default">
  Image ছোট রাখা শুধু জায়গা বাঁচানোর ব্যাপার নয়। প্রতিবার deploy-এ server-কে এই image টানতে হয়। ছোট image মানে দ্রুত deploy আর কম bandwidth খরচ।
</Callout>
...
শেষ stage-এ শুধু `public`, `.next/standalone` আর `.next/static` copy করা হয়েছে। `node_modules`, source code, build tool — কিছুই যায়নি। ওগুলো আগের stage-এ ছিল, আর সেই stage গুলো final image-এ থাকে না।

<Callout type="warning">
  `ENV HOSTNAME=0.0.0.0` লাইনটিও জরুরি। Docker প্রতিটি container-এ `HOSTNAME` নামে একটি variable বসিয়ে দেয় যার value হলো container-এর নিজের id। Next.js-এর তৈরি করা `server.js` ওই variable দেখে ঠিক করে কোন ঠিকানায় শুনবে। ফলে সে ভুল জায়গায় bind করে বসে, আর বাইরে থেকে connection আসে না।

  `0.0.0.0` মানে "সব ঠিকানায় শোনো"। Next.js-এর official Dockerfile-এও ঠিক এই দুটি লাইন আছে।
</Callout>

<Callout type="warning">
  `USER nextjs` লাইনটি বাদ দেবেন না। এটি না থাকলে আপনার app container-এর ভেতরে `root` হিসেবে চলবে। কেউ যদি app-এর কোনো দুর্বলতা কাজে লাগিয়ে ভেতরে ঢুকে পড়ে, তখন তার হাতে container-এর সব ক্ষমতা থাকবে।
</Callout>
...
এই file আপনার প্রথম রক্ষাকবচ। কেউ pull request খুললে GitHub image build করে দেখবে। Build fail করলে pull request-এ লাল X দেখাবে এবং merge করার আগেই সমস্যা ধরা পড়বে।
...
<Callout type="warning">
  `YOUR_USERNAME` আপনার GitHub username দিয়ে বদলান, এবং **সবটুকু ছোট হাতের অক্ষরে** লিখুন। আপনার username-এ বড় হাতের অক্ষর থাকলেও registry-তে নাম ছোট হাতের হতেই হবে। এই এক জায়গায় অনেকে আটকে যান।
</Callout>
...
- **`127.0.0.1:3000:3000`** — port শুধু server-এর ভেতরে খোলা, internet থেকে সরাসরি ঢোকা যাবে না। বাইরের traffic Nginx হয়ে আসবে
...
env:
  IMAGE: ghcr.io/your_username/myapp-web
...
```
image: ghcr.io/omar/myapp-web:${TAG}
                                 ↓
image: ghcr.io/omar/myapp-web:a3f9c21
```

Deploy করার মানে দাঁড়ায় শুধু ওই এক লাইন বদলে `docker compose up -d` চালানো।

আর rollback করার মানে দাঁড়ায় ওই এক লাইনে পুরোনো নামটি ফিরিয়ে আনা। Image আগে থেকেই server-এ নামানো আছে, তাই কিছু টানতেও হয় না — **প্রায় সাথে সাথেই** পুরোনো version ফিরে আসে।
...
এটি কেন দরকার, একটু ভেবে দেখুন। Script-এর তৃতীয় লাইনেই `.env`-এ নতুন TAG বসিয়ে দেওয়া হয়। এরপর যদি `docker compose pull` fail করে (registry-তে ঢুকতে না পারলে, বা image না পেলে), তাহলে `set -e`-র কারণে script সাথে সাথে থেমে যাবে।
...
<Callout type="warning">
  **Image-এর ভেতরে যা রাখেন তা সবাই পড়তে পারে।**
  ...
  ```bash filename="LOCAL — Terminal"
  docker history ghcr.io/omar/myapp-web:latest --no-trunc
  ```
...
- [ ] Server-এ এখন কয়টি file আছে বলতে পারবেন?
