...
এই section-এ "pipeline" শব্দটি বারবার আসবে, তাই আগে সেটির মানে পরিষ্কার করে নেওয়া দরকার।

Pipeline হলো কতগুলো ধাপের একটি সারি, যেগুলো **একের পর এক নিজে থেকে চলে**। আপনি শুরু করিয়ে দিলেন, বাকিটা নিজে হয়।

আপনি এখন হাতে যা করেন সেটিও আসলে একটি pipeline — শুধু সেটির প্রতিটি ধাপ আপনি নিজে চালান:

```
code লিখুন → push → ssh → pull → build → restart
```

CI/CD-র কাজ হলো এই একই সারিকে একটি file-এ লিখে রাখা, যাতে একটি machine প্রতিবার হুবহু একইভাবে চালায়।

দুটি নিয়ম মনে রাখুন:

- ধাপগুলো **উপর থেকে নিচে** চলে, একটা শেষ হলে পরেরটা
- কোনো ধাপ **fail করলে সেখানেই সব থেমে যায়** — পরের ধাপগুলো আর চলে না
...
<Callout type="default">
  নামটি এসেছে পানির পাইপ থেকে। জিনিস এক দিক দিয়ে ঢোকে, ধাপে ধাপে এগোয়, অন্য দিক দিয়ে বের হয়। মাঝখানে কোথাও আটকে গেলে সামনে আর কিছু যায় না।
</Callout>

বাস্তবে একটি pipeline মানে আপনার project-এ রাখা একটি YAML file। সেই file-এ লেখা থাকে কোন ধাপ কখন চলবে। GitHub সেটি পড়ে নিজে থেকে কাজটি করে।

## CI and CD

এবার দেখি এই pipeline-এর ভেতরে ঠিক কী কী থাকে। দুটি আলাদা কাজ, একসাথে লেখা হয় বলে অনেকে গুলিয়ে ফেলেন।
...
- Source code server-এ পড়ে থাকে। Server-এ কেউ ঢুকে পড়লে আপনার পুরো codebase আর git history তার হাতে

**পথ ২ — Runner build করে, server শুধু চালায়**

```
push → runner build করে image বানায় → registry-তে রাখে → server টেনে চালায়
```

এখানে server আর কিছু বানায় না। সে একটি তৈরি image টেনে এনে চালায়, ব্যস।
...
Runner হলো একটি **অস্থায়ী Ubuntu machine** যা GitHub আপনাকে প্রতিবার নতুন করে দেয়।
...
Runner image বানায়
      ↓  docker push
GHCR  (ghcr.io/omar/myapp-web:a3f9c21)
      ↓  docker pull
Server image টেনে চালায়
```

<Callout type="default">
  Docker Hub-ও ব্যবহার করা যায়। কিন্তু GHCR-এ একটি সুবিধা আছে — আপনার repository-র সাথে যুক্ত থাকায় GitHub Actions নিজে থেকেই login করে নিতে পারে, আলাদা কোনো password বা secret তৈরি করতে হয় না।
</Callout>
...
```
ghcr.io/omar/myapp-web:latest      ← কোনটা? বলা যাচ্ছে না
ghcr.io/omar/myapp-web:a3f9c21     ← এই commit-এর, নিশ্চিত
```
...
Workflow file আপনার project-এর ভেতরে থাকে, একটি নির্দিষ্ট folder-এ:
...
<Callout type="default">
  এখানে কোনো `compose.yaml` নেই — এটি ইচ্ছাকৃত। Compose file আর `.env` শুধু **server-এ** থাকে, repository-তে নয়। কারণ ওই দুটি file-এ server-এর নিজস্ব তথ্য আর secret থাকে।
</Callout>
...
Step দুই ধরনের হয়, এই পার্থক্যটা জানা দরকার।
...
`actions/checkout` হলো GitHub-এর নিজের লেখা একটি action যা repo-র code runner-এ clone করে। নিজে লিখতে গেলে ২০ লাইন লাগত।
...
কোনো action-কে extra তথ্য দিতে `with` ব্যবহার হয়:
...
Pipeline fail করলে GitHub-এ **Actions** tab-এ যাবেন। সেখানে:

- লাল X মানে কোনো job fail করেছে
- Job-এ click করলে step-গুলো দেখাবে
- যে step-এ লাল X, সেটিতে click করলে পুরো log খুলবে
- Log-এর **শেষ ২০ লাইন** পড়ুন — আসল error প্রায় সবসময় সেখানেই

একটি step fail করলে নিচের step-গুলো আর চলে না। তাই প্রথম লাল X-টিই আসল সমস্যা।
...
<Callout type="warning">
  **Third-party action মানে অন্যের code আপনার pipeline-এ চলছে।**
  ...
  এই ঘটনার সবচেয়ে জরুরি শিক্ষাটি অনেকে বুঝতে ভুল করেন। Attacker শুধু code বদলায়নি — সে **পুরোনো version tag গুলোও নতুন করে ওই malicious commit-এর দিকে ঘুরিয়ে দিয়েছিল**। অর্থাৎ যারা `@v44` লিখে রেখেছিলেন, তাদের কোনো লাভ হয়নি। `@v44` তখন অন্য জিনিস দেখাচ্ছিল।

  যাঁরা বেঁচে গিয়েছিলেন তাঁরা commit SHA দিয়ে pin করে রেখেছিলেন। SHA বদলানো যায় না — সেটিই একমাত্র আসল সুরক্ষা।

  তিনটি নিয়ম:

  - `@main` কখনো নয়। এটি সবচেয়ে খারাপ, কারণ এখানে attack করারও দরকার নেই — maintainer নিজেই যেকোনো সময় code বদলাতে পারেন
  - `@v4` সর্বনিম্ন মান। বেশিরভাগ ক্ষেত্রে যথেষ্ট, কিন্তু উপরের ঘটনা দেখাল যে tag ও সরানো যায়
  - **সত্যিকারের সুরক্ষা commit SHA।** সম্পূর্ণ ৪০ অক্ষরের SHA লিখতে হবে — ছোট করে লিখলে GitHub সেটি চিনতে পারে না:
  ...
  `actions/` এবং `docker/` দিয়ে শুরু হওয়া action-গুলো যথাক্রমে GitHub আর Docker-এর নিজের — এগুলোর ঝুঁকি তুলনামূলক কম, তাই এই guide-এ `@v4` ধরনের tag ব্যবহার করা হয়েছে।
</Callout>

## Quick Check

- [ ] Pipeline মানে কী, নিজের ভাষায় বলতে পারবেন?
- [ ] একটি ধাপ fail করলে পরের ধাপগুলোর কী হয়?
- [ ] Build কেন server-এ না করে runner-এ করা উচিত?
- [ ] Registry কী এবং কেন লাগে?
- [ ] Image-এ শুধু `latest` tag দিলে কী হারাবেন?
- [ ] `uses` আর `run`-এর পার্থক্য জানেন?
