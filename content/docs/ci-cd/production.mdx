...
<Callout type="warning">
  প্রতিটি app-এর নিজের `package.json` **এবং নিজের `package-lock.json`** থাকতে হবে। Repository-র root-এ কোনো `package.json` নেই।

  কারণটি Docker-এর। নিচের workflow-এ প্রতিটি image-এর build context হলো `./apps/web` বা `./apps/api`। Docker সেই folder-এর বাইরের কোনো file দেখতে পায় না। তাই lock file root-এ থাকলে `npm ci` সেটি খুঁজেই পাবে না এবং build fail করবে।

  npm workspaces ব্যবহার করলে lock file একটিই হয় এবং সেটি root-এ থাকে। তখন build context root করতে হয় আর Dockerfile-এর প্রতিটি path বদলাতে হয়। এই guide-এ সেই জটিলতায় যাচ্ছি না — দুটি app আলাদা রাখলে এখানে কম ভুল হয়।
</Callout>

<Callout type="warning">
  একই কারণে **প্রতিটি app folder-এ নিজের `.dockerignore` লাগবে**। Docker `.dockerignore` খোঁজে শুধু build context-এর root-এ। Context যেহেতু `apps/web` আর `apps/api`, repository-র root-এ রাখা `.dockerignore` এখানে কোনো কাজেই আসবে না।

  দুটি file-এই একই content:

  ```bash filename="apps/web/.dockerignore এবং apps/api/.dockerignore"
  node_modules
  .next
  dist
  .git
  ...
  এটি না থাকলে আপনার local `.env` file image-এর ভেতরে ঢুকে registry-তে চলে যাবে।
</Callout>
...
`.env` file-এ database-এর password আছে। Frontend-এর সেটি জানার কোনো দরকার নেই, তাই তাকে দেওয়াও হয়নি। প্রতিটি container শুধু ততটুকুই পাবে যতটুকু তার লাগে — কোনো container-এ কেউ ঢুকে পড়লে তখন সে যেন সবকিছু না পায়।
...
<Callout type="warning">
  ...
  Next.js এই variable গুলো **build-এর সময়** JavaScript bundle-এর ভেতরে বসিয়ে দেয়। অর্থাৎ image বানানোর মুহূর্তেই value-টি স্থায়ীভাবে ঢুকে যায়। Server-এর `.env`-এ পরে লিখলে তখন আর কিছু বদলায় না।

  এগুলো দিতে হয় build-এর সময়, workflow file-এ:
  ...
  আর `apps/web/Dockerfile`-এ builder stage-এ সেটি গ্রহণ করতে হয়:
  ...
  এই কারণেই `NEXT_PUBLIC_`-এ কখনো secret দেবেন না। এটি image-এর ভেতরে বসে যায়, আর browser-এ পাঠানো হয় — যে কেউ page source-এ দেখতে পারে।
</Callout>

`DATABASE_URL` এবং `POSTGRES_*` build-এর সময় লাগে না, তাই সেগুলো এখানেই ঠিক আছে। এগুলো container চালু হওয়ার সময় পড়া হয়।
...
Script-এর কয়েকটি অংশ ব্যাখ্যা দরকার।
...
**Image ছোট রাখুন**
`node:20-alpine` ব্যবহার করুন, `node:20` নয়। সাধারণ `node:20` image প্রায় ১ GB, alpine version ১৫০ MB-র কাছাকাছি। Multi-stage build আর `npm prune --omit=dev` তো আছেই।
...
| Disk full হয়ে গেছে | পুরোনো image জমে আছে | `docker image prune -af --filter "until=336h"` চালান, দরকারে সময় কমান |
