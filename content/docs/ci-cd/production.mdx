...
Script-এর তিনটি অংশের ব্যাখ্যা দরকার।
...
Database query রাখবেন না — তাহলে database একটু ধীর হলেই health check fail করবে আর pipeline অকারণে rollback করে বসবে।
...
প্রথমে দেখুন কোন backup ফেরাবেন:
...
POSTGRES_USER=$(grep '^POSTGRES_USER=' .env | cut -d= -f2-)
POSTGRES_DB=$(grep '^POSTGRES_DB=' .env | cut -d= -f2-)
cat ~/backups/pre-a3f9c21.sql | docker compose exec -T db psql -U "$POSTGRES_USER" "$POSTGRES_DB"
```
...
GHCR free, কিন্তু private package-এ সীমা আছে। GitHub-এর official হিসাব:

| Package | Storage | মাসে data transfer |
|---|---|---|
| Public | সীমাহীন | সীমাহীন |
| **Private (Free plan)** | **500 MB** | **1 GB** |

<Callout type="default">
  খেয়াল করুন — এটি **package**-এর visibility, repository-র নয়। দুটি আলাদা setting। Repository private রেখেও package public করা যায়, আর তখন কোনো সীমা থাকে না।

  আপনার site যদি এমনিতেই সবার জন্য খোলা হয়, package public করাই সহজ পথ। বিস্তারিত [Setup](/docs/ci-cd/setup) page-এ আছে।
</Callout>

Private রাখলে ৫০০ MB দ্রুত ভরে যেতে পারে। তিনটি কাজ করলে সমস্যা হবে না:
